...
function_params: function_param remaining_function_params { PRINT_RULE("<parametros_funcion> ::= <parametro_funcion> <resto_parametros_funcion>", RULE_FUNCTION_PARAMS); }
...
exp: exp TOK_MAS exp { PRINT_RULE("<exp> ::= <exp> + <exp>", RULE_EXP); }
    ...
    | vector_element { PRINT_RULE("<exp> ::= <elemento_vector>", RULE_EXP + 13); }
